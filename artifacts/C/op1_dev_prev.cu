...
/*
   ...
   t = H*u;
   t(3,:)
   % u3*(a3^2 + b2^2/4 + b4^2/4) + u2*(-a2*b2 + a3*b3)/2 + u4*(-a3*b3 + a4*b4)/2 + u1*(-b2^2/4) + u5*(-b4^2/4)
                        ...
                        const float *g, const float *a, const float *b, const float *s)
{
    ...
    */
    /*
    [0
     (b2*(a2 - b2))/2 - (b3*(a3 + b3))/2
     a3^2 + b2^2/2 + b3^2 + b4^2/2
     (b3*(a3 - b3))/2 - (b4*(a4 + b4))/2
     0]
     simplify(sum(H(3,:),2))
     a3^2 + (a2*b2)/2 - (a4*b4)/2
   */
    t   = g[ii] - ((u[o2]-uii)*((b[o2]*(a[o2]-b[o2]) - b[ii]*(a[ii]+b[ii]))/2) +
                   (u[o4]-uii)*((b[ii]*(a[ii]-b[ii]) - b[o4]*(a[o4]+b[o4]))/2));

    /* Subtract centre voxel multiplied by sum of the Hessian elements */
    t  -= uii*(a[ii]*a[ii] + (a[o2]*b[o2] - a[o4]*b[o4])/2);

    /* Begin to compute the denominator in the Jacobi update */
    /* Original:
    w0  = a[ii]*a[ii] + (b[o2]*b[o2] + b[o4]*b[o4])/4;
    */
    w0  = a[ii]*a[ii] + b[ii]*b[ii] + (b[o2]*b[o2] + b[o4]*b[o4])/2;
                       ...
                       const float *u, const float *a, const float *b, const float *s)
{
    ...
    */
    ...
    t   = (u[o2]-uii)*((b[o2]*(a[o2]-b[o2]) - b[ii]*(a[ii]+b[ii]))/2) +
          (u[o4]-uii)*((b[ii]*(a[ii]-b[ii]) - b[o4]*(a[o4]+b[o4]))/2);
