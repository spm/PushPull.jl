...
function ad_consistency_gpu(d::NTuple{3,Int64}, reg::Vector{<:AbstractFloat})
...
function operator_consistency_gpu(d::NTuple{3,Int64}, reg::Vector{<:AbstractFloat})
    bnd = [2 1 1; 1 2 1; 1 1 2] # Sliding boundary
    vx  = [1.2, 0.7, 1.5]
    u0  = randn(Float32,(d...,3))
    L   = registration_operator(vx, reg)
...
@testset "PushPull.jl" begin
    ...
    tol = 1e-8
    d   = (8,7,1)
    @test operator_consistency_gpu(d,[1e-3, 0.,0.,0.]) < tol
    @test operator_consistency_gpu(d,[1e-3, 1.,0.,0.]) < tol
    @test operator_consistency_gpu(d,[1e-3, 1.,9.,0.]) < tol
    @test operator_consistency_gpu(d,[1e-3, 1.,9.,1.]) < tol
    ...
    d     = (8,7,1)
    c     = 2
    f1    = CUDA.randn(Float32,(d...,c))
    f2    = CUDA.randn(Float32,(d...,c))
    phi   = CUDA.randn(Float32,(d...,3))
    phi .+= PushPull.id(phi)
    ...
    bs    = 2
    f1    = CUDA.randn(Float32,(d...,c,bs))
    f2    = CUDA.randn(Float32,(d...,c,bs))
    phi   = CUDA.randn(Float32,(d...,3,bs))
    phi .+= PushPull.id(phi)
